...
export default function Claim() {
	...
	const { isLoading, isSubmitting, error, result, submitResult, step, run, submit, reset } =
		useTwitterProof();
		...
		>
			<div style={{ width: "100%", maxWidth: "760px", display: "grid", gap: "20px" }}>
				<header style={{ textAlign: "center" }}>
					<h1 style={{ fontSize: "28px", margin: 0, fontWeight: 700 }}>Claim your tips</h1>
					<p className="help-text" style={{ marginTop: "6px" }}>
						Verify ownership of your X handle via password reset email and withdraw to any
						address.
					</p>
				</header>
				...
				>
					...
					<div>
						...
						>
							<div className="help-text" style={{ marginBottom: "8px" }}>
								{file ? `✓ Selected: ${file.name}` : "Click to choose .eml file"}
							</div>
							<button
								type="button"
								className="link-cta"
					...
					{submitResult && (
						...
						>
							<div style={{ marginBottom: 6 }}>
								Withdrawal submitted from {truncateMiddle(submitResult.accountAddress)}.
							</div>
							<div>
								Tx:{" "}
								<a
					...
					<div style={{ display: "flex", gap: 10 }}>
						{!result ? (
							<button className="nav-cta" onClick={handleGenerate} disabled={!canGenerate} style={{ flex: 1 }}>
								{isLoading ? "Generating..." : "Generate Proof"}
							</button>
						) : (
							<>
								<button className="link-cta" onClick={handleReset} disabled={isSubmitting} style={{ flex: 1 }}>
									Reset
								</button>
								<button className="nav-cta" onClick={submit} disabled={isSubmitting} style={{ flex: 1 }}>
